...
from .serializers import (
    UGSeatMatrixSerializer,
    PGFeeDetailsSerializer,
    OldClosingRankSerializer,
    PrivateCollegeSerializer,
    NIRFUniversityRankingSerializer,
    LoginSerializer,
    AllotmentSerializer,
    ClosingRankSerializer,
    SeatMatrixSerializer,
    FeeStipendBondSerializer,
)
from django_filters.rest_framework import DjangoFilterBackend
from django.contrib.auth import get_user_model
from rest_framework import generics, permissions
...
class SignupView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        # Build a plain dict so we can inject signup_ip BEFORE serializer.save()
        # This ensures the signal fires with signup_ip already on the user object.
        data = dict(request.data)
        data["signup_ip"] = get_client_ip(request)
...
class RankPredictorView(APIView):
    ...
    def get(self, request):
        ...
        if not user_rank or not user_rank.isdigit():
            return Response(
                {"error": "Please provide a valid rank (e.g. ?rank=2534)"}, status=400
            )

        user_rank = int(user_rank)
        colleges = RankPredictionCollege.objects.filter(
            closing_rank__gte=user_rank
        ).order_by("closing_rank")[:100]
        serializer = RankPredictionCollegeSerializer(colleges, many=True)
        return Response(serializer.data)
...
class CSVUploadView(APIView):
    ...
    def post(self, request, type):
        ...
        if category not in CATEGORIES:
            return Response(
                {
                    "detail": "Invalid or missing category. Must be one of the predefined categories."
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"detail": "Could not decode file — please upload UTF-8 CSV."},
                status=400,
            )
        ...
        if not rows:
            return Response(
                {"detail": "CSV appears empty or headers missing."}, status=400
            )
                ...
                if type == "allotment":
                    ...
                    for r in rows:
                        data = {
                            ...
                            "round": _get(r, "Round", "round"),
                            "ai_rank": _get(
                                r, "AI Rank", "Ai Rank", "ai_rank", "ai_rank"
                            ),
                            "state": _get(r, "State", "state"),
                            "institute": _get(r, "Institute", "institute"),
                            "course": _get(r, "Course", "course"),
                            ...
                            "fee": _get(r, "Fee", "fee"),
                            "stipend_year1": _get(
                                r, "Stipend Year 1", "Stipend_Year_1", "stipend_year1"
                            ),
                            "bond_years": _get(r, "Bond Years", "bond_years"),
                            "bond_penalty": _get(r, "Bond Penalty", "bond_penalty"),
                            "beds": _get(r, "Beds", "beds"),
                    ...
                    for r in rows:
                        data = {
                            ...
                            "quota": _get(r, "Quota"),
                            "open_seats": _to_int(
                                _get(r, "Open", "Open Seats", "open")
                            ),
                            "open_pwd": _to_int(
                                _get(r, "Open PwD", "Open_PwD", "open_pwd")
                            ),
                            "gen_ews": _to_int(
                                _get(
                                    r,
                            ...
                            "st_pwd": _to_int(_get(r, "ST PwD", "st_pwd")),
                            "total_seats": _to_int(
                                _get(r, "TotalSeats", "Total Seats", "total_seats")
                            ),
                        }
                        objs.append(SeatMatrix(**data))
                    SeatMatrix.objects.bulk_create(objs, batch_size=500)
        ...
        return Response(
            {"detail": f"{len(rows)} rows imported into {type} / {category}"},
            status=201,
        )
        ...
        for _, row in df.iterrows():
            ...
            objs.append(
                AllotmentData(
                    ...
                    stipend_year1=row["Stipend Year 1"],
                    bond_years=(
                        row["Bond Years"] if not pd.isna(row["Bond Years"]) else None
                    ),
                    bond_penalty=row["Bond Penalty"],
                    beds=row["Beds"],
                )
...
class ForgotPasswordAPIView(APIView):
    ...
    def post(self, request):
        ...
        user_name = (
            user.first_name
            if user.first_name
            else (user.username if user.username else "User")
        )

        # Send email via Google Apps Script
        try:
            apps_script_url = getattr(settings, "PASSWORD_RESET_APPS_SCRIPT_URL", "")
            if not apps_script_url:
                raise Exception("PASSWORD_RESET_APPS_SCRIPT_URL not configured")
            ...
            resp = http_requests.post(apps_script_url, json=payload, timeout=30)
            resp_data = resp.json()
            ...
            if resp_data.get("success"):
                ...
                raise Exception(resp_data.get("error", "Apps Script returned failure"))
...
from .serializers import UGAllotmentDataSerializer
from .pagination import StandardResultsSetPagination
...
class UGAllotmentDataList(generics.ListAPIView):
    ...
    serializer_class = UGAllotmentDataSerializer
...
from core.serializers import UGAllotmentDataSerializer

# class UGAllotmentDataViewSet(viewsets.ReadOnlyModelViewSet):
#     queryset = UGAllotmentData.objects.all()
#     serializer_class = UGAllotmentDataSerializer
from rest_framework import viewsets, filters
from django_filters.rest_framework import DjangoFilterBackend
...
class UGAllotmentFilterOptionsView(APIView):
    """
    Returns distinct values for every filterable field.
    Called once on page mount to populate dropdowns & round pills.
    Very fast — only DISTINCT queries, no row data returned.
    """

    def get(self, request):
        qs = UGAllotmentData.objects

        rounds = qs.values_list("round", flat=True).distinct().order_by("round")
        categories = (
